...
int carregarDadosCSV(const char *nomeArquivo)
{
    ...
    if (cartas == NULL)
    {
        perror("GAME: Erro ao alocar memória");
        fclose(arquivo);
        return 0;
...
void lerCartasMultiplayer()
{
    ...
    // Alocação:
    cartas_multiplayer = (Carta *)malloc(capacidade * sizeof(Carta));
    if (cartas_multiplayer == NULL)
    {
        perror("GAME: Erro ao alocar memória");
        fclose(arquivo);
        exit(1);
    ...
    while (fgets(linha, max_linhas, arquivo))
    {
        ...
        cartas_multiplayer[tamanho++] = c; // Cartas na posição do tamnho atual recebe o c
    } // Percorre todo o arquivo
...
bool verificaOuCriaArquivoBinario(const char *nomeArquivo)
{
    ...
    if (arquivo)
    {
        escreverLog("Arquivo binário não idenficado, gerando um novo.");
    ...
    escreverLog("Erro ao criar o novo.");
...
int carregarDadosBIN(const char *nomeArquivo)
{
    ...
    if (!arquivo)
    {
        escreverLog("Erro ao abrir o arquivo binário.");
    ...
    {
        escreverLog("Erro ao ler do arquivo binário.");
