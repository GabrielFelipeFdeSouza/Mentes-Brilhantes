...
void gerenciarCartas(RenderTexture2D *target, Music *musica, Sound sons[], Carta **cartas)
{
    ...
    while (tela == 1)
    {
        ...
        {
            resaltaBotoes(botoes_gerenciador, 7, *musica, sons[0], botoes_nulos_gerenciador, 2); // Função que checa se o usuário esta sob um btn
        }
        ...
        if (submenu_tela == 1)
        {
            ...
            {
                *cartas = (Carta *)realloc(*cartas, (quantidade_cartas + 1) * sizeof(Carta)); // Aumenta o tamanho do vetor de cartas
